...
class RVCPipeline:
    ...
    def preflight_check(self):
        ...
        # 1. Check repo root
        if not os.path.exists(self.repo_root) or not os.path.exists(
            os.path.join(self.repo_root, "infer")
        ):
    ...
    def extract_dataset(self):
        ...
        if not os.path.exists(model_dataset_dir):
            print(
                f"⚠️ Warning: Expected extracted folder {model_dataset_dir} not found. Ensure tarball contains a folder named {self.model_name}"
            )
    ...
    def train_pipeline(self):
        ...
        default_threads = max(
            1, os.cpu_count() // len(self.config.get("gpu", "0").split("-"))
        )
        ...
        if not log_path:
            ...
            if not os.path.exists(log_path):
                log_path = os.path.join(
                    self.repo_root, f"logs/{self.model_name}/train_stdout.log"
                )
    ...
    def backup(self):
        ...
        weight_files = glob.glob(
            os.path.join(self.repo_root, f"assets/weights/{self.model_name}*.pth")
        )
        ...
        if os.path.exists(log_dir):
            ...
            if g_files and d_files:
                latest_g = sorted(
                    g_files, key=lambda x: int(x.split("_")[-1].split(".")[0])
                )[-1]
                latest_d = sorted(
                    d_files, key=lambda x: int(x.split("_")[-1].split(".")[0])
                )[-1]
                try:
                    shutil.copy2(latest_g, backup_dir)
                    shutil.copy2(latest_d, backup_dir)
                    print(
                        f"✅ Copied latest resume checkpoints: {os.path.basename(latest_g)}, {os.path.basename(latest_d)}"
                    )
                except Exception as e:
        ...
        if weights:
            ...
            for w in weights:
                if "G_" not in os.path.basename(w) and "D_" not in os.path.basename(w):
                    ...
                    print(
                        f"✅ Restored weight: {os.path.basename(w)} to assets/weights/"
                    )
        else:
            print("⚠️ No inference weights found in backup.")

        # Restore index
        indices = glob.glob(f"{backup_dir}/added_IVF*.index") + glob.glob(
            f"{backup_dir}/{self.model_name}_IVF*.index"
        )
        if indices:
            ...
            print(
                f"✅ Restored index: {os.path.basename(indices[0])} to logs/{self.model_name}/"
            )
        else:
    ...
    def inference(self):
        ...
        # ── Task-list mode (per-file f0_up_key) ─────────────────────────────
        # When the orchestrator passes a tasks.json, we use that instead of a
        # directory scan so each file gets its individually computed pitch key.
        task_list_path = inf_config.get("task_list", None)
        f0_key_map: dict[str, int] = {}

        if task_list_path and os.path.exists(task_list_path):
            with open(task_list_path) as _f:
                task_entries = json.load(_f)
            audio_files = [e["path"] for e in task_entries]
            f0_key_map = {e["path"]: e["f0_up_key"] for e in task_entries}
            is_batch = len(audio_files) > 1
            if not audio_files:
                print(f"❌ task_list is empty: {task_list_path}")
                sys.exit(1)
        else:
            # ── Legacy: directory or single-file mode ────────────────────────
            input_audio_path = inf_config.get("input_audio", "")
            if not input_audio_path:
                print("❌ No input_audio or task_list provided in config.")
                sys.exit(1)

            is_batch = os.path.isdir(input_audio_path)
            if is_batch:
                audio_files = [
                    os.path.join(input_audio_path, f)
                    for f in os.listdir(input_audio_path)
        ...
        # Set expected env vars for RVC WebUI code
        os.environ["weight_root"] = "assets/weights"
        os.environ["index_root"] = "assets/indices"
        os.environ["rmvpe_root"] = "assets/rmvpe"
        os.environ["rmvpe_proj_root"] = "assets/rmvpe"

        # Build RVC Config
        rvc_cfg = RVCConfig()

        # Restore sys.argv now that Config is initialized
        sys.argv = old_argv

        import torch

        if torch.cuda.is_available() and self.config.get("gpu"):
        ...
        if not index_name:
            ...
            if indices:
                ...
                tgt = os.path.join(
                    f"logs/{self.model_name}", os.path.basename(index_name)
                )
        ...
        def get_params(f_in):
            ...
            # ── Per-file f0_up_key override (from tasks.json) ────────────
            if f_in in f0_key_map:
                params["f0_up_key"] = f0_key_map[f_in]
            elif randomize and "f0_up_key" in rng:
                params["f0_up_key"] = random.randint(
                    rng["f0_up_key"][0], rng["f0_up_key"][1]
                )

            # ── Randomize remaining params ─────────────────────────────────────
            if randomize:
                if rng.get("index_rate"):
                    params["index_rate"] = random.uniform(*rng["index_rate"])
                if rng.get("filter_radius"):
                    params["filter_radius"] = random.randint(*rng["filter_radius"])
                if rng.get("rms_mix_rate"):
                    params["rms_mix_rate"] = random.uniform(*rng["rms_mix_rate"])
                if rng.get("protect"):
                    params["protect"] = random.uniform(*rng["protect"])

            return params

        def process_single_file(f_in):
            params = get_params(f_in)
            ...
            if "Success" in msg and audio_opt[0] is not None:
                ...
                return {"success": False, "file": os.path.basename(f_in), "error": msg}

        # Determine worker count for GPU concurrency
        # 4 is usually a sweet spot for 8GB VRAM to overlap RMVPE and VAE
        max_workers = 1 if not is_batch else min(4, os.cpu_count() or 1)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in tqdm(
                as_completed(futures), total=len(audio_files), desc="RVC Inference"
            ):
                res = future.result()
                if res["success"]:
                    if is_batch:
                        # Extract the base params for logging
                        log_entry = {
                            k: v
                            for k, v in res.items()
                            if k not in ["success", "error"]
                        }
                        batch_log_rows.append(log_entry)
                else:
                    print(f"\n❌ Failed: {res['file']} - {res['error']}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command",
        choices=["train", "infer", "backup", "restore", "analyze"],
        help="Command to run",
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Only run preflight checks (for train command)",
    )
    parser.add_argument(
        "--log-file", help="Path to specific log file to analyze (override config)"
    )
